...
from .forms import LoginForm, RegistrationForm, EditProfileForm, PostForm, ResetPasswordRwquestForm, ResetPasswordForm
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Congratuation, you are a registered user.')
        return redirect(url_for('login'))
