...
from app.api.errors import bed_request
...
def create_user():
    ...
    if 'username' not in data or 'email' not in data or 'password' not in data:
        return bed_request('must include username, email and password fields')
    if User.query.filter_by(username=data['username']).first():
        return bed_request('please use a different username')
    if User.query.filter_by(email=data['email']).first():
        return bed_request('please use a different email address')
    user = User()
...
def update_user(id):
    ...
    if 'email' in data and data['email'] != user.email and User.query.filter_by(email=data['email']).first():
        return bed_request('plaes use a different email address')
